...
interface UnitSystem {
  ...
  scale: number;
  /** `$INSUNITS` code: 1 inches, 4 millimeters. */
  insunits: number;
}
 ...
 *
 * This must agree with `toFormatUnit`, which drives the LABELS layer text. When
 * they disagree the file scales its geometry one way and prints its dimensions
 * another, which is invisible until someone cuts to a label. The dropped
 * `metric-cm` setting did exactly that, and the fix was to delete it; if a unit
 * is ever added back, both sides move together or neither does.
 */
...
function provenance(input: SheetDxfInput, units: UnitSystem): string[] {
  const unitName = units.insunits === 1 ? 'in' : 'mm';
  return [
